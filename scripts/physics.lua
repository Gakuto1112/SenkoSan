---@class PhysicsClass 物理演算（もどき）を制御するクラス
...
---@field PhysicsClass.EnablePyhsics boolean 物理演算を有効にするかどうか：1. 尻尾, 2. 髪飾り
...
PhysicsClass.EnablePyhsics = {true, true}
...
events.RENDER:register(function ()
	...
	if (not renderer:isFirstPerson() or client:hasIrisShader()) and (PhysicsClass.EnablePyhsics[1] or PhysicsClass.EnablePyhsics[2]) then
		...
		if playerPose == "FALL_FLYING" then
			...
			if PhysicsClass.EnablePyhsics[1] then
				tailRot = vectors.vec3(math.clamp(VelocityAverage[1] * 80, rotLimit[1][1][1], rotLimit[1][1][2]), math.clamp(-VelocityAverage[4] * 0.1, rotLimit[1][2][1], rotLimit[1][2][2]), 0)
			end
			tail:setRot(tailRot)
			...
			if PhysicsClass.EnablePyhsics[2] then
				hairAccessoryLineRot = vectors.vec3(math.clamp(60 - VelocityAverage[1] * 80, rotLimit[2][1][1], rotLimit[2][1][2]), 0, math.clamp(-VelocityAverage[4] * 0.1, rotLimit[2][2][1], rotLimit[2][2][2]))
			end
		...
		elseif playerPose == "SWIMMING" then
			local tailRot = vectors.vec3(0, 0, 0)
			if PhysicsClass.EnablePyhsics[1] then
				tailRot = vectors.vec3(math.clamp(VelocityAverage[1] * 320, rotLimit[1][1][1], rotLimit[1][1][2]), math.clamp(-VelocityAverage[4] * 0.2, rotLimit[1][2][1], rotLimit[1][2][2]), 0)
			end
			tail:setRot(tailRot)
			if CostumeClass.CurrentCostume == "DISGUISE" then
				disguiseTail:setRot(tailRot)
			...
			local hairAccessoryLineRot = vectors.vec3(0, 0, 0)
			if PhysicsClass.EnablePyhsics[2] then
				hairAccessoryLineRot = vectors.vec3(math.clamp(60 - VelocityAverage[1] * 320, rotLimit[2][1][1], rotLimit[2][1][2]), 0, math.clamp(-VelocityAverage[4] * 0.2, rotLimit[2][2][1], rotLimit[2][2][2]))
			end
			for _, modelPart in ipairs(hairAccessoryLines) do
				modelPart:setRot(hairAccessoryLineRot)
			end
