...
SitDown = General.instance({
	...
	---@diagnostic disable-next-line: undefined-field
	return player:getPose() == "STANDING" and player:isOnGround() and not player:isInWater() and not player:isInLava() and player:getFrozenTicks() == 0 and not player:getVehicle() and not player:isMoving() and Hurt.Damaged == "NONE" and not Warden.WardenNearby
