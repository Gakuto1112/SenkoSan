...
HairCut = {
	...
	new = function ()
		local instance = General.instance(HairCut, AnimationAction, function ()
		...
		instance.ChairBlock = models.models.hair_cut:newBlock("hair_cut.chair"):block("minecraft:oak_stairs"):pos(8, -6, -4):rot(0, 180, 0) --椅子代わりの階段ブロックのレンダータスク
		instance.ScissorsItem = models.models.hair_cut.Avatar.Body.Arms.RightArm.RightArmBottom.Scissors:newItem("hair_cut.scissors"):item("minecraft:shears"):pos(2, 0, -3):rot(-90, 45, 0):scale(0.5, 0.5, 0.5):enabled(false) --鋏のアイテムレンダータスク。
	...
	stop = function (self)
		...
		self.ScissorsItem:enabled(false)
		sounds:playSound("entity.item.pickup", player:getPos(), 1, 0.5)
	end,
	...
	onAnimationTick = function (self)
		...
		if self.AnimationCount > 0 then
			...
			elseif self.AnimationCount == 383 then
				...
				self.ScissorsItem:enabled(true)
				sounds:playSound("entity.item.pickup", player:getPos(), 1, 0.5)
