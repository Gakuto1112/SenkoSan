...
**このREADMEには本来、大量の画像（約68MB）を添付していますが、通信量の軽減のため、必要最低限以外の画像は添付されおりません。画像を確認するには、それぞれの画像のリンクをクリックするか、[README（完全版）](./.github/workflows/README_full.md)をご覧ください。**
...
- 複数の衣装チェンジができます。どのような衣装があるかは[衣装カタログ](衣装一覧.md)をご覧ください。
  ...
  [[画像] メイド服A](README_images/メイド服A.jpg)
...
- 水に触れると濡れてしまいます。
  - 水から上がると身震いして体に付いた水滴を飛ばします（[設定](#アバター設定)でオフにできます）。
    ...
    [[画像] ブルブル](README_images/ブルブル.gif)
...
| 項目 | 推奨設定 | 備考 |
| - | - | - |
| キー設定 | 「尻尾フリフリ」や「お耳ピクピク」と同じキー割り当てを行わない | |
