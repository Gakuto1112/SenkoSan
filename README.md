...
**このREADMEには本来、大量の画像（約57MB）を添付していますが、通信量の軽減のため、必要最低限以外の画像は添付されおりません。全ての画像を確認したい方は[README（完全版）](./.github/workflows/README_full.md)をご覧ください。**
...
### アクション2-3. 自動ブルブルの切り替え
[アバター設定](#アバター設定について)の``ConfigClass.AutoShake``の値を変更できます。ただし、**アバターが再読み込みされる度に値が元に戻ります。**

### アクション2-4. 防具非表示の切り替え
[アバター設定](#アバター設定について)の``ConfigClass.HideArmor``の値を変更できます。ただし、**アバターが再読み込みされる度に値が元に戻ります。**
